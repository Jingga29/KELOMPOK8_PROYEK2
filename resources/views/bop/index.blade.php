...
@section('main-content')
    <div class="container">
        <h2>Data BOP</h2>
        <a href="{{ route('bop.create') }}" class="btn btn-primary">Masukkan BOP</a>
    ...
    .action-buttons {
        ...
        <table class="table table-custom">
            ...
            <tbody>
                @php
                    $totalBOP = 0; // Initialize the totalBOP variable
                @endphp
            
                @foreach($bops as $bop)
                    <tr>
                        <td style="padding: 5px; width: 25%;">{{ $bop->tanggal }}</td>
                        <td style="padding: 5px; width: 25%;">{{ $bop->item }}</td>
                        <td style="padding: 5px; width: 25%;">{{ number_format($bop->nominal, 0, ',', '.') }}</td>
                        <td style="padding: 5px; width: 25%;">
                            <a href="{{ route('bop.edit', $bop->id) }}" class="btn btn-warning btn-custom"><i class="fa-solid fa-pen-to-square"></i></a>
                            <form action="{{ route('bop.destroy', $bop->id) }}" method="post" style="display: inline;">
                                @csrf
                                @method('delete')
                                <button type="submit" class="btn btn-danger btn-custom" onclick="return confirm('Apakah Anda yakin ingin menghapus data BOP?')"><i class="fa-solid fa-trash-can"></i></button>
                            </form>
                        </td>
                    </tr>
                    @php
                        $totalBOP += $bop->nominal;
                    @endphp
                @endforeach
            </tbody>
            <tr>
                <td colspan="2"></td>
                <td style="padding: 5px; width: 25%;">Total: {{ number_format($totalBOP, 0, ',', '.') }}</td>
                <td></td>
            </tr>
        </table>
    </div>
